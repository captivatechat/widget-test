...
async function applyBackground() {
  document.getElementById("status").innerHTML =
    `Please wait while we process your request... <img src="img/loading.gif" alt="loading" width="10px" height="10px">`;
  ...
  if (webRadio.checked) {
    ...
    const response = await fetch(
      "https://widget-preview-server.do.captivatechat.ai/screenshot",
      {
        method: "POST",
        headers: {
  ...
  if (imageRadio.checked) {
    ...
    if (imageFile) {
      ...
      const response = await fetch(
        "https://widget-preview-server.do.captivatechat.ai/upload",
        {
          method: "POST",
          body: formData,
        },
...
function runWidget() {
  var captivate_id = DOMPurify.sanitize(
